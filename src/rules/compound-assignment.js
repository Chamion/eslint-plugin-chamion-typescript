const identifierNodeEquals = (a, b) =>
...
const isCompoundableOperator = Array.prototype.includes.bind(
  COMPOUNDABLE_BINARY_OPERATORS
);

const SYMMETRICAL_BINARY_OPERATORS = ["*", "&", "^", "|"];
const isSymmetricalOperator = Array.prototype.includes.bind(
  SYMMETRICAL_BINARY_OPERATORS
);
        ...
        AssignmentExpression: (node) => {
          ...
          const fix = canFix
            ? (fixer) => {
                ...
                const binaryExpressionText =
                  leftText + binaryOperator + rightText;
    ...
    default:
      return {
        AssignmentExpression: (node) => {
          ...
          const identifierPosition = binaryExpressionIdentifierPosition(
            node.left,
            binaryExpression
          );
          if (
            !isCompoundableOperator(binaryExpression.operator) ||
          ...
          const fix = (fixer) => {
            ...
            const assignmentText =
              leftText + assignmentOperatorText + rightText;
            return fixer.replaceText(node, assignmentText);
          };
          context.report({
            node,
