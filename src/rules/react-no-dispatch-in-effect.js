const isHook = (name) => (expression) =>
  isIdentifier(name)(expression) ||
  (expression.type === "MemberExpression" &&
    isIdentifier("React")(expression.object) &&
    isIdentifier(name)(expression.property));

const isStateHook = (expression) =>
  isHook("useState")(expression) || isHook("useReducer")(expression);

const isIdentifier = (name) => (expression) =>
  expression.type === "Identifier" && expression.name === name;

const isStateHookCall = (expression) =>
  expression.type === "CallExpression" && isStateHook(expression.callee);
...
const hasSameRange = (a, b) =>
  a.range[0] === b.range[0] && a.range[1] === b.range[1];

const getNearestParentCall = (scopeNode) => {
  if (scopeNode.type === "CallExpression") return scopeNode;
  else if (scopeNode.parent == null) return null;
  else return getNearestParentCall(scopeNode.parent);
};

const collectUseEffects = (scope) =>
  scope.references
    .filter(
      (reference) =>
        isIdentifier("useEffect")(reference.identifier) ||
...
function create(context) {
  ...
  const visit = (node) => {
    ...
    const dispatchCalls = scope.childScopes
      ...
      .filter(
        (reference) => reference.identifier.parent.type === "CallExpression"
      )
    ...
    const violations = dispatchCalls.filter((dispatchCall) =>
      useEffectBodyRanges.some((range) => isInRange(range)(dispatchCall))
    );
    violations.forEach(
      (violation) =>
        void context.report({
          ...
          message:
            "Dispatch call in useEffect callback may cause unnecessary rerenders",
        })
    );
  };
  return {
