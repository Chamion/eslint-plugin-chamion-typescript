...
const isHook = (...names) => (expression) => isIdentifier(...names)(expression) ||
    (expression.type === "MemberExpression" &&
        isIdentifier("React")(expression.object) &&
        isIdentifier(...names)(expression.property));
const isIdentifier = (...names) => (expression) => expression.type === "Identifier" && names.includes(expression.name);
const rangeEquals = (a, b) => a.range[0] === b.range[0] && a.range[1] === b.range[1];
const refersTo = (...nodes) => (reference) => reference.resolved != null &&
    reference.resolved.identifiers.some((resolved) => nodes.some((node) => rangeEquals(resolved, node)));
const getImmediateExecutionPathScopes = (scope) => {
    const { childScopes } = scope;
    const immediateExecutionChildScopes = childScopes.filter((scope) => scope.type === "block");
    return [
        scope,
...
const create = (context) => {
    ...
    const getReturnCallbacks = (functionNode) => {
        if (functionNode.body.type !== "BlockStatement")
            return ["ArrowFunctionExpression", "FunctionExpression"].includes(functionNode.body.type)
                ? [functionNode.body]
                : [];
        const immediateExecutionPath = getImmediateExecutionPathScopes(scopeManager.acquire(functionNode));
        const functionChildScopes = immediateExecutionPath.flatMap((scope) => scope.childScopes.filter((childScope) => childScope.type === "function"));
    ...
    const visit = (node) => {
        ...
        const getHookInvocations = (...names) => scope.references.filter((reference) => isHook(...names)(reference.identifier) &&
        ...
        const dispatches = getHookInvocations("useState", "useReducer")
            ...
            .filter((maybeDeclarator) => maybeDeclarator.type === "VariableDeclarator")
            .filter(({ id }) => id.type === "ArrayPattern" &&
            id.elements[1] != null &&
            id.elements[1].type === "Identifier")
            .map(({ id }) => id.elements[1]);
        if (dispatches.length === 0)
            return;
        const useEffectCallbacks = getHookInvocations("useEffect", "useLayoutEffect")
        ...
        if (useEffectCallbacks.length === 0)
            return;
        [
            ...useEffectCallbacks,
    ...
    const visitCallback = (dispatches, parentScope) => (node) => {
        ...
        const dispatchCalls = immediateExecutionPath
            ...
            .filter((reference) => refersTo(...dispatches)(reference) &&
            reference.identifier.parent &&
            reference.identifier.parent.type === "CallExpression")
            .map((reference) => reference.identifier.parent);
        dispatchCalls.forEach((call) => context.report({
            node: call,
            message: "Unexpected state dispatch call in effect callback. Resolve the whole state transition in one rerender.",
        }));
    };
    return {
