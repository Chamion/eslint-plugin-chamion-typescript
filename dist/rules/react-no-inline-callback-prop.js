...
const isCallbackDeclarator = (node) => node.type === "VariableDeclarator" &&
    node.init != null &&
...
function create(context) {
    ...
    const visitComponentOrHook = (node) => {
        ...
        const callbackVariables = scope.variables.filter(({ defs }) => defs.length > 0 &&
    ...
    return {
        ...
        JSXOpeningElement: (node) => {
            ...
            const attributeExpressions = attributes
                .map(({ value }) => value)
                .filter((value) => value != null && value.type === "JSXExpressionContainer")
                .map(({ expression }) => expression);
            attributeExpressions
                .filter(({ type }) => type === "ArrowFunctionExpression" || type === "FunctionExpression")
                .forEach((expression) => {
                context.report({
                    node: expression,
                    message: "Inline callback expression has a unique identity on each render. Wrap it in useCallback.",
                });
            });
            attributeExpressions
                ...
                const identifiedVariable = callbacksInComponentsOrHooks
                    .filter((variable) => isInRange(variable.scope.block.range)(expression))
                    .find((variable) => variable.identifiers.length > 0 &&
